  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const botMessage: Message = {
        id: Date.now() + 1,
        text: response.data?.reply || "I'm sorry, I received an empty response. Could you try again?",
        sender: "bot",
      };
      ...
      const errorMessage: Message = {
        id: Date.now() + 1,
        text: "Sorry, I'm having trouble connecting to the server. Please check your network or try again later.",
        sender: "bot",
      };
        ...
        <ScrollArea className="h-full p-4" ref={scrollAreaRef}>
          <div className="max-w-2xl mx-auto space-y-6 pb-4">
            ...
            {isLoading && (
              ...
              >
                <LoaderCircle className="animate-spin w-4 h-4 flex-none" />
                <span className="text-xs">
                  {isSlowResponse
                    ? "Waking up the assistant, this can take up to a minute on first use..."
                    : "Typing..."}
                </span>
              </motion.div>
            )}
